	...
	Tags
  {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			if (projMatrix[3][3] == 0) scale = lerp(abs(scale) * (1 - _PerspectiveFilter), scale, abs(dot(UnityObjectToWorldNormal(input.normal.xyz), normalize(WorldSpaceViewDir(vert)))));
